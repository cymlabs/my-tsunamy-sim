...
const simButtons = [btnPause, btnReset, btnSnapshot, btnBackpedal, btnAdvance];
...
function switchPane(paneId, { updateHash = true } = {}) {
  ...
  const label = paneNames[paneId] || paneId.replace('-', ' ');
  setStatus(`Showing ${label}`);
  if (updateHash) {
    const newHash = `#${paneId}`;
...
function syncPaneFromHash() {
  ...
  const validPane = panes && Array.from(panes).some((pane) => pane.id === `pane-${hash}`);
  const target = validPane ? hash : 'landing';
  switchPane(target, { updateHash: false });
  return target;
}
...
function validateTextures(device) {
  try {
    device.createTexture({ size: [4, 4], format: 'r16float', usage: GPUTextureUsage.STORAGE_BINDING | GPUTextureUsage.TEXTURE_BINDING });
    device.createTexture({ size: [4, 4], format: 'rgba16float', usage: GPUTextureUsage.STORAGE_BINDING | GPUTextureUsage.TEXTURE_BINDING });
  } catch (err) {
    throw new Error('Required storage texture formats are unavailable on this GPU/browser.');
  }
}
...
function wirePager() {
  ...
  searchInput?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      ...
      setStatus(`Searching for ${query || activeName}`);
    }
  });
...
function wireShare() {
  ...
  btnCopyLink?.addEventListener('click', () => {
    ...
    const activePane = document.querySelector('.nav-btn.active')?.dataset.pane;
    if (activePane) {
      url.hash = activePane;
    }
    if (navigator.clipboard?.writeText) {
      ...
      try {
        ...
        setStatus('Copy this link manually: ' + url.toString(), false);
      }
    }
  });
  ...
  btnGenerateReport?.addEventListener('click', () => {
    const activePane = document.querySelector('.nav-btn.active')?.dataset.pane;
    const url = new URL(window.location.href);
    if (activePane) url.hash = activePane;
    const report = `${currentSummary()} • Snapshot generated at ${new Date().toLocaleString()} • Link: ${url.toString()}`;
    if (navigator.clipboard?.writeText) {
      navigator.clipboard
        .writeText(report)
...
wireExit();
wireNoGpu();
const initialPane = syncPaneFromHash();
setStatus(`Showing ${paneNames[initialPane] || initialPane}`);
setControlsEnabled(false);
bootWebGPU();
